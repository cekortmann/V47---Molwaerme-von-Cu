...
R, t, U, I, dT, T, dt, E, cp, a, cv= np.genfromtxt('cv.txt', unpack=True, skip_header=1, skip_footer=13)
           # Normierung 
...
plt.plot(T, cv, 'xr', markersize=6 , label = 'Messdaten', alpha=0.5)
#plt.plot(xx, g(xx, *para), '-b', linewidth = 1, label = 'Ausgleichsfunktion', alpha=0.5)
plt.xlabel(r'$T \, / \, \mathrm{K}$')
plt.ylabel(r'$C_V \, / \,\mathrm{J/(molK)}$')
plt.legend(loc="best")                  # legend position
plt.grid(True)                          # grid style
#plt.xlim(22, 40)
#plt.ylim(-0.05, 1.05)

plt.savefig('build/cv170.pdf', bbox_inches = "tight")
plt.clf() 
